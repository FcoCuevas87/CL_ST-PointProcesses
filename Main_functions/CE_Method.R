...
my.GeoCorrFct <- function(x = x, corrmodel = corrmodel, param = param){
  if(corrmodel != "none"){
    if(param$nugget == 0){
      result <- GeoModels::GeoCorrFct(x = x, corrmodel = corrmodel, param = param)$corr
    }else{
      result <- GeoModels::GeoCorrFct(x = x, corrmodel = corrmodel, param = param)$corr 
      result[x == 0] <- 1
    }
...
GRF.CE.Sim <- function(mean.val ,xrange, yrange, M,N, corrmodel, param, nsim = 1, max.ext = 8){
  fail <- TRUE
  approx.flag <- FALSE
  pow <- seq(1, max.ext, 1)
  k0 <- 1
...
# ################
# 
# param.B <- list(mean = 0, smooth = 2.5, scale = 0.25, sill  = 1, nugget = 0.0)
# Covariate.sim.B <- GRF.CE.Sim(nsim = 1, c(0,1), c(0,1), M = 100, N = 100, 
#                               corrmodel = "Matern", 
#                               param = param.B, 
#                               mean.val = 0, 
#                               max.ext = 5)
# fields::image.plot( matrix(Covariate.sim.B$X, 100, 100, byrow = T) )
# 
# 
# ##############
# #### Wave ####
# ##############
# param.B <- list(mean = 0, scale = 0.01, sill  = 1, nugget = 0)
# Covariate.sim.B <- GRF.CE.Sim(nsim = 1, c(0,1), c(0,1), M = 100, N = 100, 
#                               corrmodel = "Wave", 
#                               param = param.B, 
#                               mean.val = 0, 
